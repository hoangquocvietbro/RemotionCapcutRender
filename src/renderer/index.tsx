import { ISize, IDesign, ITransition, IItem } from "@designcombo/types";
...
export const RenderVideo = ({design, size}: {design: IDesign; size: ISize}) => {
	...
	return (
        ...
        if (group.length === 1) {
          const item = design.trackItemsMap[group[0]] as IItem;
        ...
        return (
          <TransitionSeries key={index} from={from}>
            {populateTransitionIds(group).map((id, index) => {
              ...
              if (index % 2 === 0) {
                const item = design.trackItemsMap[id] as IItem;
								// return TransitionSequenceItem[item.type](item, { fps, timeLine });
                return TransitionSequenceItem[item.type](item, { fps });
              } 
